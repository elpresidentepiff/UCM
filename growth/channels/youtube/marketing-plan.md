# UCM YouTube Marketing Plan

## Commercial Role

Use YouTube as UCM's searchable evidence and authority library. Shorts create discovery; longer videos answer buyer questions and support sales conversations.

## Priority Audiences

- Office and facilities managers.
- Property and block managers.
- Landlords and letting agents.
- Buyers responsible for technical environments.
- Prospects comparing cleaning contractors.

## Monthly Baseline

- Four Shorts adapted from approved vertical assets.
- One or two longer videos built from founder, service or case-study material.
- One playlist update per priority service campaign.

## Content Architecture

- `UCM Service Guides`: scope, process and buyer FAQs.
- `Carlos on Site`: founder judgement and practical lessons.
- `Proof Passport Stories`: approved problem, delivery and outcome.
- `London Property Care`: borough and development-specific advice.
- `Technical Cleaning`: IT equipment, data centers and specialist work.

## Publishing Standard

- Short: 9:16, 1080x1920, 15-60 seconds, captions and one CTA.
- Long: 16:9, clear audio, practical structure, chapters when useful and no filler intro.
- Thumbnail: real subject, readable short title and consistent UCM identity.
- Description: service summary, proof context, quote URL and tracking.
- Pinned comment: one relevant next step.

## Conversion

- Primary CTA: relevant service or quote landing page.
- Tracking: `utm_source=youtube&utm_medium=organic_social`.
- Use videos in proposals and follow-up emails when they answer the prospect's actual concern.

## Media Engine Readiness

The existing YouTube adapter is live-ready but cannot be trusted until credentials are rotated, real uploads are tested privately and duplicate protection is verified. Publishing remains human-approved.

## KPIs

- Search impressions and click-through rate.
- Audience retention and watch time.
- Returning viewers.
- Landing-page sessions and quote completions.
- Qualified leads and influenced contracts.

## First 90 Days

### Days 1-30

- Optimise channel identity, About text, links and playlists.
- Publish founder introduction, quote process and office-cleaning Short.

### Days 31-60

- Publish one approved service walkthrough and one technical-cleaning guide.
- Test question-led titles against generic service titles.

### Days 61-90

- Publish the first approved case-study video.
- Use retention and lead data to choose the next service series.

